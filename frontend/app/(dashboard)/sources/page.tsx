...
export default function Page() {
  ...
  const handleTriggerSelectedScrape = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data?.detail || `Scrape request failed (${response.status})`,
        );
      }
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800">
              <tr>
                ...
                <th className="p-3 text-left">Impact Score</th>
                ...
                scrapedItems.map((item, index) => (
                  <Fragment key={`${item.url}-${index}`}>
                    <tr
                      ...
                      onClick={() =>
                        setExpandedRow(
                          expandedRow === item.url ? null : item.url,
                        )
                      }
                    >
                      ...
                      <td className="p-3">
                        <span
                          className={`rounded-full px-2 py-1 text-xs font-semibold ${impactClasses(item.impact_score)}`}
                        >
                          {item.impact_score}
                        </span>
                      </td>
